...
<style>
    ...
    .operation-group-header { border-bottom: 2px solid #e5e7eb; }
    .table-container { scrollbar-width: thin; scrollbar-color: #cbd5e1 #f8fafc; overflow-x: auto; max-width: 100%; }
    .section-masters { background-color: #f8fafc; }
    .section-delegation { background-color: #f0fdf4; }
    .section-survey { background-color: #eff6ff; }
    .section-material { background-color: #fffbeb; }
    .section-installation { background-color: #f5f3ff; }
...
<div class="card mb-6 shadow-sm border-gray-200">
    <div class="card-body p-4">
        <form id="filterForm" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3">
            ...
            <div>
                <select id="cityFilter" name="city" class="block w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-blue-500 focus:border-blue-500">
                    <option value="">All Cities</option>
                </select>
            </div>
            <div>
                <select id="stateFilter" name="state" class="block w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-blue-500 focus:border-blue-500">
                    <option value="">All States</option>
                </select>
            </div>
            <div>
                <select id="statusFilter" name="activity_status" class="block w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-blue-500 focus:border-blue-500">
            ...
            <div>
                <select id="surveyStatusFilter" name="survey_status" class="block w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-blue-500 focus:border-blue-500">
...
<div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
    <div class="flex items-center gap-3">
        ...
        <span id="recordCount" class="px-2 py-1 bg-gray-100 text-gray-600 text-xs font-bold rounded-full">0 Records</span>
    </div>
    <div class="flex flex-wrap items-center gap-2">
        <button id="refreshBtn" class="btn btn-secondary !px-3 font-bold text-xs whitespace-nowrap">
            <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path></svg>
            Refresh
        </button>
        <button id="exportBtn" class="btn btn-success !px-3 font-bold text-xs whitespace-nowrap inline-flex items-center">
            <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
            <span id="exportText">Export</span>
        </button>
        <a href="bulk_upload.php" class="btn btn-info !px-3 font-bold text-xs whitespace-nowrap">
            <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"></path></svg>
            Bulk Import
        </a>
        <button onclick="openModal('createSiteModal')" class="btn btn-primary !px-3 font-bold text-xs ring-2 ring-blue-100 whitespace-nowrap">
            <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg>
            Add Site
        </button>
    </div>
</div>
...
<div class="card shadow-md border-gray-200 overflow-hidden relative" id="tableCard">
        ...
        <table class="min-w-full divide-y divide-gray-200" id="sitesTable">
            <thead class="bg-gray-50 border-b border-gray-200">
                ...
                <tr class="text-[11px] font-bold text-gray-500 uppercase tracking-wider text-center">
                    <th colspan="5" class="px-4 py-3 bg-gray-100 border-r border-gray-200 text-gray-700">1. Masters Data</th>
                    <th colspan="2" class="px-4 py-3 bg-green-50 border-r border-gray-200 text-green-800">2. Delegation</th>
                    <th colspan="4" class="px-4 py-3 bg-blue-50 border-r border-gray-200 text-blue-800">3. Survey Status</th>
                    <th colspan="4" class="px-4 py-3 bg-yellow-50 border-r border-gray-200 text-yellow-800">4. Material Part</th>
            ...
            <tbody id="sitesTableBody" class="bg-white divide-y divide-gray-100 italic text-gray-400">
                <tr><td colspan="19" class="py-10 text-center">Dashboard initialization...</td></tr>
            </tbody>
        </table>
    </div>
    ...
    <div id="emptyState" class="hidden py-16 text-center">
        <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                ...
                <tr class="hover:bg-gray-50 transition-colors text-xs">
                    ...
                    <td class="px-4 py-3 border-r border-gray-100 section-masters">
                        <div class="flex items-center gap-1">
                            ...
                            <button onclick="delegateSite(${site.id})" class="p-1.5 text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="Delegate Site"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg></button>
