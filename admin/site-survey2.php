...
$stmt = $db->prepare("SELECT id, survey_status, submitted_date, is_draft, approval_status FROM dynamic_survey_responses WHERE delegation_id = ? ORDER BY id DESC LIMIT 1");
...
if ($existingSurvey && !$existingSurvey['is_draft'] && $existingSurvey['approval_status'] !== 'rejected') {
...
<div id="dynamicFormApp" class="professional-table bg-white">
    ...
    <template v-if="!loading && !error">
        ...
        <div v-if="isLocked" class="bg-green-50 border-l-4 border-green-400 p-6 m-6 rounded-r-lg">
            <div class="flex items-center">
        ...
        <div class="p-6" :class="{'pb-32': surveyStatus !== 'not_started'}">
            <form @submit.prevent="submitSurvey" enctype="multipart/form-data" :class="{'pointer-events-none opacity-60': surveyStatus === 'not_started', 'pointer-events-none opacity-75': isPreviewMode && !isLocked, 'pointer-events-none': isLocked}">
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <button v-if="surveyStatus === 'not_started'" 
                                        @click="startSurvey" 
                                        type="button"
                                        class="inline-flex items-center px-6 py-3 border border-transparent text-sm font-medium rounded-md text-white bg-green-600 hover:bg-green-700 transition-colors shadow-lg">
                                    <svg class="w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM9.555 7.168A1 1 0 008 8v4a1 1 0 001.555.832l3-2a1 1 0 000-1.664l-3-2z" clip-rule="evenodd"></path>
                                    </svg>
                                    Start Survey
                                ...
                                <button v-if="surveyStatus === 'in_progress'" 
                                        @click="endSurvey" 
                                        type="button"
                                        class="inline-flex items-center px-6 py-3 border border-transparent text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 transition-colors shadow-lg">
                                    <svg class="w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 20 20">
...
createApp({
    ...
    methods: {
        ...
        async startSurvey() {
            try {
                ...
                if (data.success) {
                    ...
                    alert('Failed to start survey: ' + data.message);
                }
            } catch (err) {
                console.error(err);
            ...
            try {
                ...
                const response = await fetch('../api/survey-progress.php?action=save_draft', {
                    ...
                    body: new URLSearchParams({
                        ...
                        site_master: JSON.stringify(siteMaster)
                    })
                });
        ...
        async endSurvey() {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    alert('Survey ended. Please review your answers before submitting.');
                } else {
                    alert('Failed to end survey: ' + data.message);
                }
            } catch (err) {
                console.error(err);
        ...
        async checkSurveyStatus() {
            ...
            console.log('Delegation ID:', '<?php echo $delegationId; ?>');
            ...
            try {
                ...
                if (data.success && data.survey) {
                    ...
                    console.log('Existing survey found:', survey);
                    this.surveyResponseId = survey.id;
                    
                    if (survey.approval_status === 'approved') {
                        console.log('Survey is approved - locking');
                        this.isLocked = true;
                        this.surveyStatus = 'approved';
                        this.isPreviewMode = true;
                    } else if (survey.is_draft == 1) {
                        if (survey.survey_ended_at) {
                            console.log('Survey is ended - preview mode');
                            this.surveyStatus = 'ended';
                            this.isPreviewMode = true;
                        } else {
                            console.log('Survey is in progress - resuming');
                            this.surveyStatus = 'in_progress';
                            // Start auto-save
                            this.autoSaveInterval = setInterval(() => this.saveDraft(true), 30000);
                        }
                        ...
                        if (survey.form_data) {
                            const savedData = JSON.parse(survey.form_data);
                    ...
                    console.log('No existing survey found - status remains:', this.surveyStatus);
                }
            } catch (err) {
    ...
    async mounted() {
        ...
        console.log('Initial surveyStatus:', this.surveyStatus);
        console.log('Initial loading:', this.loading);
        console.log('Initial error:', this.error);
        ...
        console.log('After loadSurvey - surveyStatus:', this.surveyStatus);
        console.log('After loadSurvey - loading:', this.loading);
        console.log('After loadSurvey - error:', this.error);
        
        // Check if there's an existing survey draft or submitted survey
        await this.checkSurveyStatus();
        
        console.log('After checkSurveyStatus - surveyStatus:', this.surveyStatus);
