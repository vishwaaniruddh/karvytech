...
$query = "
    SELECT s.*, c.name as customer_name,
           COALESCE(NULLIF(TRIM(CONCAT_WS(' ', u.first_name, u.last_name)), ''), u.username, 'System') as creator_name,
           (SELECT COUNT(*) FROM dynamic_survey_responses sr WHERE sr.survey_form_id = s.id) as response_count
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        <?php foreach ($surveys as $index => $survey): ?>
                            <tr id="form-row-<?php echo $survey['id']; ?>" class="transition-all duration-500">
                                <td>
                                    <span class="text-xs font-bold text-gray-400">#<?php echo $index + 1; ?></span>
                                </td>
                                <td>
                                ...
                                <td>
                                    <div class="flex flex-col space-y-1">
                                        <span class="inline-flex items-center w-max px-2.5 py-0.5 rounded-full text-xs font-medium <?php echo $survey['form_type'] === 'survey' ? 'bg-indigo-100 text-indigo-800' : 'bg-orange-100 text-orange-800'; ?>">
                                            <?php echo ucfirst($survey['form_type']); ?>
                                        </span>
                                        <?php if ($survey['customer_name']): ?>
                                            <div class="text-[10px] font-semibold text-gray-500 uppercase tracking-tight"><?php echo htmlspecialchars($survey['customer_name']); ?></div>
                                        <?php else: ?>
                                            <span class="text-[10px] text-gray-400 italic">Global Form</span>
                                        <?php endif; ?>
                                    </div>
                                </td>
                                <td>
                                    <div class="text-sm font-medium text-gray-700"><?php echo htmlspecialchars($survey['creator_name']); ?></div>
                                    <div class="text-[10px] text-gray-500"><?php echo date('M d, Y', strtotime($survey['created_at'])); ?></div>
                                </td>
                                <td>
                                    <div class="flex flex-col">
                                        <div class="text-sm font-bold text-gray-800"><?php echo $survey['response_count']; ?></div>
                                        <div class="text-[10px] text-gray-400 uppercase">Responses</div>
                                    </div>
                                </td>
                                <td>
                                ...
                                <td>
                                    <div class="flex space-x-2">
                                        ...
                                        <a href="form-responses.php?id=<?php echo $survey['id']; ?>" class="btn btn-sm btn-secondary hover:bg-emerald-50 hover:text-emerald-600 border-gray-200" title="View Responses">
                                        ...
                                        <button onclick="deleteSurvey(<?php echo $survey['id']; ?>, '<?php echo htmlspecialchars(addslashes($survey['title'])); ?>')" class="btn btn-sm btn-danger hover:bg-rose-600 border-none shadow-sm" title="Delete Form">
            ...
            fetch(`../../api/surveys_v2.php?action=delete&id=${id}`, { method: 'DELETE' })
                ...
                .then(async data => {
                    if (data.success) {
                        ...
                        await showConfirm(
                            'Form Deleted',
                            `The form "${title}" has been permanently removed from the system.`,
                            { confirmType: 'success', confirmText: 'Done', hideCancel: true }
                        );
                    } else {
