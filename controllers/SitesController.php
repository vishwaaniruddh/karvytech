...
class SitesController extends BaseController {
    ...
    public function index() {
        ...
        // Get overall statistics (GLOBAL - no filters applied)
        $overallStats = $this->siteModel->getOverallStatistics('', []);
    ...
    public function store() {
        try {
            ...
            $data = [
                ...
                'contact_person_number' => trim($_POST['contact_person_number'] ?? ''),
                'contact_person_email' => trim($_POST['contact_person_email'] ?? ''),
                'created_by' => $currentUser['username'],
                'site_ticket_id' => $siteTicketId
            ];
            
            // Validate contact person fields using validation helper
            $validationErrors = [];
            
            // Validate phone number if provided
            if (!empty($data['contact_person_number'])) {
                $phoneValidation = validatePhoneNumber($data['contact_person_number'], false);
                if (!$phoneValidation['valid']) {
                    $validationErrors[] = 'Contact Person Number: ' . $phoneValidation['message'];
                } else {
                    // Use formatted phone number
                    $data['contact_person_number'] = $phoneValidation['formatted'];
                }
            }
            
            // Validate email if provided
            if (!empty($data['contact_person_email'])) {
                $emailValidation = validateEmail($data['contact_person_email'], false);
                if (!$emailValidation['valid']) {
                    $validationErrors[] = 'Contact Person Email: ' . $emailValidation['message'];
                }
            }
    ...
    public function delete($id) {
        try {
            ...
            if ($success) {
                // Log the action
                ErrorHandler::logUserAction('SOFT_DELETE_SITE', 'sites', $id, $site, null);
                
                return $this->jsonResponse([
                    'success' => true,
                    'message' => 'Site moved to trash successfully'
                ]);
            } else {
                return $this->jsonResponse([
                    'success' => false,
    ...
    public function permanentDelete($id) {
        try {
            ...
            if (!$currentUser || $currentUser['role'] !== 'superadmin') {
                return $this->jsonResponse([
                    'success' => false,
                    'message' => 'Unauthorized. Only superadmin can permanently delete sites.'
                ], 403);
            }
            ...
            $success = $this->siteModel->permanentDelete($id);
            
            if ($success) {
                // Log the action
                ErrorHandler::logUserAction('PERMANENT_DELETE_SITE', 'sites', $id, $site, null);
                ...
                return $this->jsonResponse([
                    'success' => false,
                    'message' => 'Failed to permanently delete site'
                ], 500);
            }
            
        } catch (Exception $e) {
            Logger::error('Site permanent deletion failed', ['error' => $e->getMessage(), 'site_id' => $id]);
            return $this->jsonResponse([
                'success' => false,
                'message' => 'An error occurred while permanently deleting the site'
            ], 500);
        }
    }
    ...
    public function restore($id) {
        try {
            $success = $this->siteModel->restore($id);
            
            if ($success) {
                // Log the action
                ErrorHandler::logUserAction('RESTORE_SITE', 'sites', $id, null, null);
            ...
            Logger::error('Site restoration failed', ['error' => $e->getMessage(), 'site_id' => $id]);
            return $this->jsonResponse([
                'success' => false,
                'message' => 'An error occurred while restoring the site'
            ], 500);
        }
    }
