...
// Require authentication (allow both admin and vendor)
Auth::requireAuth();
...
if (!$installationId) {
    ...
    if ($currentUser && $currentUser['role'] === 'vendor') {
        header('Location: ../contractor/installations.php');
    } else {
...
if (!$installation) {
    ...
    $currentUser = Auth::getCurrentUser();
    if ($currentUser && $currentUser['role'] === 'vendor') {
        header('Location: ../contractor/installations.php?error=Installation not found');
    } else {
        header('Location: ../admin/installations/index.php?error=Installation not found');
    }
    exit;
}
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between">
        <div class="flex-1">
            <div class="flex items-center space-x-4 mb-2">
                <?php
                $currentUser = Auth::getCurrentUser();
                $backUrl = ($currentUser && $currentUser['role'] === 'vendor') 
                    ? '../contractor/installations.php' 
                    : '../admin/installations/index.php';
                ?>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div class="space-y-3">
            <div class="flex justify-between">
                ...
                <span class="font-medium"><?php echo htmlspecialchars($installation['vendor_company_name'] ?: $installation['vendor_name'] ?? 'Unknown'); ?></span>
            </div>
            <div class="flex justify-between">
...
<div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div>
            <span class="text-gray-500 text-sm">Company Name</span>
...
<div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-8">
    ...
    <div class="space-y-6">
        <?php foreach ($dailyWork as $day): 
            ...
            // Get photos/videos for this day from installation_progress_attachments
            ...
            ");
            $stmt->execute([$installationId, "Day {$day['day_number']} -%"]);
            $dayPhotos = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ?>
                        ...
                        <?php if (!empty($dayPhotos)): ?>
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <?php foreach ($dayPhotos as $photo): 
                                    // Fix path - if it doesn't start with http or /, prepend BASE_URL
                                ...
                                ?>
                                    <div class="relative group">
                                        <?php if ($photo['file_type'] === 'photo' || strpos($photo['mime_type'], 'image') !== false): ?>
                                            <img src="<?php echo htmlspecialchars($photoPath); ?>" 
                                                 alt="Work photo" 
                                                 class="w-full h-24 object-cover rounded-lg shadow-sm cursor-pointer hover:shadow-md transition-shadow"
                                                 onclick="openImageModal('<?php echo htmlspecialchars($photoPath); ?>', '<?php echo htmlspecialchars($photo['original_name']); ?>')">
                                        <?php elseif ($photo['file_type'] === 'video' || strpos($photo['mime_type'], 'video') !== false): ?>
                                            <video src="<?php echo htmlspecialchars($photoPath); ?>" 
                                                   class="w-full h-24 object-cover rounded-lg shadow-sm cursor-pointer hover:shadow-md transition-shadow"
                                                   onclick="openVideoModal('<?php echo htmlspecialchars($photoPath); ?>', '<?php echo htmlspecialchars($photo['original_name']); ?>')">
                                        <?php endif; ?>
                                        ...
                                        <div class="absolute bottom-0 left-0 right-0 bg-black bg-opacity-50 text-white text-xs p-2 rounded-b-lg opacity-0 group-hover:opacity-100 transition-opacity">
                                            <div class="truncate"><?php echo htmlspecialchars($photo['original_name']); ?></div>
                                            <div class="text-xs"><?php echo date('M j, g:i A', strtotime($photo['uploaded_at'])); ?></div>
                                            <?php if ($photo['attachment_type'] === 'daily_work_material'): ?>
                                                <div class="text-xs text-yellow-300">Material Photo</div>
                                            <?php else: ?>
                                                <div class="text-xs text-blue-300">Site Photo</div>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                <?php endforeach; ?>
                                    ...
                                    <div class="space-y-2">
                                        <?php foreach ($attachments as $attachment): 
                                            ...
                                            if (!preg_match('/^(https?:\/\/|\/)/i', $attachmentPath)) {
                                                $attachmentPath = BASE_URL . '/' . ltrim($attachmentPath, '/');
                                            }
                                        ?>
                                            <div class="flex items-center space-x-2 p-2 bg-white rounded border">
                                                <?php if (strpos($attachment['mime_type'], 'image') === 0): ?>
                                                    <img src="<?php echo htmlspecialchars($attachmentPath); ?>" 
                                                         ...
                                                         onclick="openImageModal('<?php echo htmlspecialchars($attachmentPath); ?>', '<?php echo htmlspecialchars($attachment['original_name']); ?>')">
                                                <?php else: ?>
                                                ...
                                                <a href="<?php echo htmlspecialchars($attachmentPath); ?>" 
...
// Load daily work images dynamically
document.addEventListener('DOMContentLoaded', function() {
    fetch('get-daily-work-images.php?installation_id=<?php echo $installationId; ?>')
    .then(response => response.json())
    .then(data => {
        console.log('Daily work images loaded:', data);
        
        if (data.success && data.attachments) {
            Object.keys(data.attachments).forEach(dayNumber => {
                const container = document.getElementById(`day-${dayNumber}-photos`);
                if (container) {
                    const photos = data.attachments[dayNumber];
                    ...
                    if (photos.length > 0) {
                        ...
                        photos.forEach(photo => {
                            ...
                            const photoPath = baseUrl + '/' + photo.file_path;
                            const isVideo = photo.file_type === 'video' || photo.mime_type.includes('video');
                            const typeLabel = photo.attachment_type === 'daily_work_material' ? 'Material Photo' : 'Site Photo';
                            const typeLabelClass = photo.attachment_type === 'daily_work_material' ? 'text-yellow-300' : 'text-blue-300';
                            
                            const photoHtml = `
                                <div class="relative group">
                                    ${isVideo ? 
                                        `<video src="${photoPath}" class="w-full h-24 object-cover rounded-lg shadow-sm cursor-pointer hover:shadow-md transition-shadow" controls></video>` :
                                        `<img src="${photoPath}" alt="Work photo" class="w-full h-24 object-cover rounded-lg shadow-sm cursor-pointer hover:shadow-md transition-shadow" onclick="openImageModal('${photoPath}', '${photo.original_name}')">`
                                    }
                                    <div class="absolute bottom-0 left-0 right-0 bg-black bg-opacity-50 text-white text-xs p-2 rounded-b-lg opacity-0 group-hover:opacity-100 transition-opacity">
                                        <div class="truncate">${photo.original_name}</div>
                                        <div class="text-xs">${new Date(photo.uploaded_at).toLocaleString()}</div>
                                        <div class="text-xs ${typeLabelClass}">${typeLabel}</div>
                                    </div>
                                </div>
                            `;
    ...
    .catch(error => {
        console.error('Error loading daily work images:', error);
    });
});
</script>
