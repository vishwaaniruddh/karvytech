...
<div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-8">
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between">
        <div class="flex-1">
            ...
            <p class="mt-2 text-lg text-gray-600">Site: <span class="font-semibold text-blue-600"><?php echo htmlspecialchars($response['site_id'] ?? 'Unknown'); ?></span></p>
        </div>
        <div class="mt-6 lg:mt-0 lg:ml-6">
            <a href="view-survey2.php?id=<?php echo $responseId; ?>" class="inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">
...
<div id="editSurveyApp" class="professional-table bg-white">
                        ...
                        <div v-if="field.field_type === 'file'">
                            ...
                            <div v-if="hasExistingFiles(field.id)" class="mb-3">
                                ...
                                <div class="grid grid-cols-2 md:grid-cols-3 gap-2 mb-2">
                                    <div v-for="(file, index) in getExistingFiles(field.id)" :key="index" class="relative group">
                                        <div v-if="isImageFile(file)" class="border rounded relative">
                                            <img :src="'../' + file.file_path" :alt="file.original_name" class="w-full h-24 object-cover rounded">
                                            <button 
                                                type="button"
                                                @click="triggerDelete(field.id, index)"
                                        ...
                                        <div v-else class="border rounded h-24 flex flex-col items-center justify-center bg-gray-50 relative">
                                            <svg class="w-8 h-8 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"></path>
                                            </svg>
                                            <span class="text-xs text-gray-600 mt-1 px-1 text-center truncate w-full">{{ file.original_name }}</span>
                                            <button 
                                                type="button"
                                                @click="triggerDelete(field.id, index)"
                                                class="absolute -top-2 -right-2 w-6 h-6 bg-red-500 text-white rounded-full hover:bg-red-600 transition-colors flex items-center justify-center shadow-lg">
    ...
    <div v-if="showDeleteModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm transition-all">
        <div class="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6 transform transition-all scale-100 border border-gray-100">
            ...
            <p class="text-gray-500 text-center mb-6">Do you really want to delete this? This action cannot be undone.</p>
            <div class="flex space-x-3">
                <button @click="showDeleteModal = false" class="flex-1 px-4 py-2 border border-gray-300 text-gray-700 font-medium rounded-xl hover:bg-gray-50 transition-colors">
                    No, Cancel
                </button>
                <button @click="confirmDelete" class="flex-1 px-4 py-2 bg-red-600 text-white font-medium rounded-xl hover:bg-red-700 shadow-lg shadow-red-200 transition-all">
                    Yes, Delete
                </button>
            </div>
        </div>
