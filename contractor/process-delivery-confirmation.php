...
try {
    ...
    $cleanPhone = null;
    if (!empty($_POST['received_by_phone'])) {
        $phoneValidation = validatePhoneNumber($_POST['received_by_phone'], false);
    ...
    $deliveryData = [
        ...
        'received_by_phone' => $cleanPhone, // Use validated and cleaned phone number
    ...
    // Update dispatch status based on delivery completeness
    ...
    foreach ($itemConfirmations as $confirmation) {
        // Get original sent quantity from dispatch
        $boqItemId = $confirmation['boq_item_id'];
        ...
        foreach ($dispatchItems as $dispatchItem) {
            if ($dispatchItem['boq_item_id'] == $boqItemId) {
                $totalSent += $dispatchItem['quantity_dispatched'];
                $totalReceived += $confirmation['received_quantity'];
                
                if ($confirmation['received_quantity'] < $dispatchItem['quantity_dispatched']) {
    ...
    // Determine final dispatch status
    $finalStatus = 'delivered';
    if ($isPartialDelivery) {
        $finalStatus = 'partially_delivered';
        
        // Update material request status to partially fulfilled instead of completed
    ...
    $inventoryModel->updateDispatchStatus($dispatchId, ['dispatch_status' => $finalStatus]);
    
    // **CRITICAL FIX**: Add received materials to installation quantities
    if (!empty($itemConfirmations)) {
        ...
        // Get installation ID from site_id
        $installation = $installationModel->getInstallationBySiteAndVendor($materialRequest['site_id'], $vendorId);
        
        if ($installation) {
            foreach ($itemConfirmations as $confirmation) {
                $boqItemId = $confirmation['boq_item_id'];
                $receivedQuantity = $confirmation['received_quantity'];
                
                if ($receivedQuantity > 0) {
                    // Get BOQ item details to get material name
                    require_once __DIR__ . '/../models/BoqItem.php';
                    $boqModel = new BoqItem();
                    $boqItem = $boqModel->find($boqItemId);
                    
                    if ($boqItem) {
                        // Add received materials to installation quantities
                        $materialUsageModel->addReceivedMaterialsToInstallation(
                            $installation['id'],
                            $boqItem['item_name'],
                            $receivedQuantity,
                            $boqItem['unit'],
                            "Received from dispatch #{$dispatchId} - Request #{$requestId}"
                        );
                    }
                }
            }
