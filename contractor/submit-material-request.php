...
try {
    ...
    // Try to match material name with BOQ items for inventory tracking
    $boqItemId = null;
    $itemCode = null;
    $unit = 'units';
    
    try {
        $boqItem = $boqModel->findByItemName($materialName);
        if ($boqItem) {
        ...
        // If BOQ item not found, continue without it (will be treated as non-inventory item)
    }
